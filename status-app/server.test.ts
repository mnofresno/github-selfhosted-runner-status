...
test('createServer exposes cleanup admin routes and cleanup status snapshots', async () => {
  ...
  const fleetCleanupResult = {
    ...
    removedStacks: [],
    pruneResult: { imagePrune: { SpaceReclaimed: 1 }, buildCachePrune: { SpaceReclaimed: 2 }, volumePrune: { skipped: true } },
    reconciledTargets: [],
    errors: [],
  };
  const globalCleanupResult = {
    mode: 'global',
    startedAt: '2026-04-15T12:00:00.000Z',
    finishedAt: '2026-04-15T12:01:00.000Z',
    durationMs: 60000,
    pruneResult: { imagePrune: { SpaceReclaimed: 3 }, buildCachePrune: { SpaceReclaimed: 4 }, volumePrune: { skipped: true } },
    statusAtRun: { targets: [] },
  };

  const { server, baseUrl } = await startTestServer([], {
    runFleetCleanupFn: async () => fleetCleanupResult,
    runGlobalCleanupFn: async () => globalCleanupResult,
    getCleanupStatusFn: () => ({
      maintenanceRunning: false,
      fleet: { running: false, lastRunAt: fleetCleanupResult.finishedAt, lastStartedAt: fleetCleanupResult.startedAt, lastResult: fleetCleanupResult, lastError: null },
      global: { running: false, lastRunAt: globalCleanupResult.finishedAt, lastStartedAt: globalCleanupResult.startedAt, lastResult: globalCleanupResult, lastError: null },
    }),
  });
  ...
  try {
    ...
    assert.deepEqual(result.body.global.lastResult, globalCleanupResult);

    result = await requestJson(baseUrl, '/api/admin/cleanup/fleet', { method: 'POST' });
    assert.equal(result.response.status, 200);
    assert.deepEqual(result.body, fleetCleanupResult);

    result = await requestJson(baseUrl, '/api/admin/cleanup/global', { method: 'POST' });
    assert.equal(result.response.status, 200);
    assert.deepEqual(result.body, globalCleanupResult);
  } finally {
    clearStatusSnapshotCache();
    server.close();
    await once(server, 'close');
