...
test('pruneGlobalResources requests image and build cache pruning only', async () => {
  const calls = [];
  ...
  const result = await pruneGlobalResources(docker);

  assert.equal(calls.length, 2);
  assert.match(calls[0].path, /^\/images\/prune\?filters=/);
  assert.deepEqual(calls[0].options, { method: 'POST' });
  assert.match(calls[1].path, /^\/build\/prune\?filters=/);
  assert.deepEqual(result, {
    imagePrune: { path: calls[0].path },
    buildCachePrune: { path: calls[1].path },
    volumePrune: { skipped: true, reason: 'disabled-by-default' },
  });
});

test('executeFleetCleanupPlan removes stale stacks, prunes globally, and reconciles targets', async () => {
  const calls = [];
  const plan = {
    staleManagedStacks: [
      {
...
test('cleanup runtime exposes per-mode state snapshots and lock behavior', async () => {
  const runtime = createCleanupRuntime();
  runtime.fleet.running = true;
  runtime.fleet.lastRunAt = '2026-04-15T00:00:00.000Z';
  runtime.fleet.lastResult = { ok: true };

  const snapshot = getCleanupStatus(runtime);
  assert.equal(snapshot.fleet.running, true);
  assert.equal(snapshot.fleet.lastRunAt, '2026-04-15T00:00:00.000Z');
