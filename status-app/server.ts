...
type CreateServerOptions = {
  ...
  runGlobalCleanupFn?: typeof runGlobalCleanup;
  ...
  return withCleanupLock(cleanupRuntime, async () => {
    ...
    try {
      ...
      const executeResult = await executeFleetCleanupPlan({
        ...
        pruneGlobalResourcesFn: () => pruneGlobalResources(docker),
...
async function runGlobalCleanup(targets, options: any = {}) {
  const {
    getStatusFn = getStatus,
    getCachedStatusFn = getCachedStatus,
  } = options;
  ...
  return withCleanupLock(cleanupRuntime, async () => {
    const startedAt = new Date().toISOString();
    cleanupRuntime.global.running = true;
    cleanupRuntime.global.lastStartedAt = startedAt;
    cleanupRuntime.global.lastError = null;

    try {
      const status = await getCachedStatusFn(targets, getStatusFn);
      const pruneResult = await pruneGlobalResources(docker);
      clearStatusSnapshotCache();
      clearGithubStatusCache();
      ...
      cleanupRuntime.global.lastRunAt = finishedAt;
      cleanupRuntime.global.lastResult = result;
      return result;
    } catch (error) {
      const finishedAt = new Date().toISOString();
      cleanupRuntime.global.lastRunAt = finishedAt;
      cleanupRuntime.global.lastError = error.message;
      cleanupRuntime.global.lastResult = {
        mode: 'global',
        startedAt,
        finishedAt,
        error: error.message,
      ...
      cleanupRuntime.global.running = false;
    }
  });
}
...
function createServer(initialTargets, options: CreateServerOptions = {}) {
  ...
  const {
    ...
    runGlobalCleanupFn = runGlobalCleanup,
  } = options;
  ...
  app.post('/api/admin/cleanup/global', asyncRoute(async (_req, res) => {
    try {
      const result = await runGlobalCleanupFn(targets, {
        getStatusFn,
        getCachedStatusFn: getCachedStatus,
      });
      res.json(result);
...
module.exports = {
  ...
  resetCleanupRuntime, snapshotCleanupStatus, runFleetCleanup, runGlobalCleanup,
};
