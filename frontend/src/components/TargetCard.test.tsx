...
describe('TargetCard', () => {
  ...
  it('reconciles a target from the target card', async () => {
    const user = userEvent.setup();
    render(
      <TargetCard
        target={baseTarget}
    ...
    await user.click(screen.getByRole('button', { name: 'Reconcile target' }));
    expect(api.reconcileTarget).toHaveBeenCalledWith('fleet-a');
  });
