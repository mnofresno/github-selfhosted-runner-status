import type { CleanupRunResult, CleanupStatus, FleetStatus, Target } from '../types';
...
type FleetDashboardProps = {
  ...
  onCleanupFleet: () => Promise<void>;
  onCleanupGlobal: () => Promise<void>;
};
...
function describeCleanupRun(result: CleanupRunResult | null | undefined) {
  if (!result) {
    return 'Never run';
  }
  if (result.skipped) {
  ...
  const staleCount = result.plan?.staleManagedStacks.length || 0;
  const removedCount = result.removedStacks?.length || 0;
  const reconciledCount = result.reconciledTargets?.length || 0;
  return `${new Date(result.finishedAt).toLocaleString()} · ${staleCount} stale, ${removedCount} removed, ${reconciledCount} reconciled`;
}

function summarizeCleanupErrors(result: CleanupRunResult | null | undefined) {
  if (!result?.errors?.length) {
    return 'None';
  }
  return result.errors.map((error) => error.error).join(' | ');
}
...
export function FleetDashboard({
  ...
  return (
    <section className="dashboard-grid">
      ...
      <section className="card cleanup-panel">
        <div className="section-head section-head-tight">
          <div>
            <h2>Cleanup control</h2>
            <p className="muted compact">Fleet cleanup and global Docker pruning are separated so the fleet can recover without manual Docker deletion.</p>
          </div>
          <div className="toolbar">
            <button type="button" disabled={busy} onClick={() => void onCleanupFleet()}>
              Cleanup fleet
            </button>
            <button type="button" disabled={busy} onClick={() => void onCleanupGlobal()}>
              Cleanup global
            </button>
          </div>
        </div>
        <div className="cleanup-summary-grid">
          <div className="metric">
            <span>Fleet last run</span>
            <strong>{describeCleanupRun(cleanupStatus?.fleet.lastResult)}</strong>
          </div>
          <div className="metric">
            <span>Global last run</span>
            <strong>{describeCleanupRun(cleanupStatus?.global.lastResult)}</strong>
          </div>
          <div className="metric">
            <span>Fleet stale stacks</span>
            <strong>{cleanupStatus?.fleet.lastResult?.plan?.staleManagedStacks.length || 0}</strong>
          </div>
          <div className="metric">
            <span>Fleet removed</span>
            <strong>{cleanupStatus?.fleet.lastResult?.removedStacks?.length || 0}</strong>
          </div>
          <div className="metric">
            <span>Global prune</span>
            <strong>{cleanupStatus?.global.lastResult?.pruneResult ? 'done' : 'idle'}</strong>
          </div>
          <div className="metric">
            <span>Last error</span>
            <strong>{cleanupStatus?.fleet.lastError || cleanupStatus?.global.lastError || 'none'}</strong>
          </div>
        </div>
        <div className="cleanup-detail-grid">
          <div>
            <h3>Fleet details</h3>
            <p className="muted compact">Errors: {summarizeCleanupErrors(cleanupStatus?.fleet.lastResult)}</p>
            <p className="muted compact">
              Ignored resources: {cleanupStatus?.fleet.lastResult?.plan?.ignoredResources.length || 0}
            </p>
          </div>
          <div>
            <h3>Global details</h3>
            <p className="muted compact">Errors: {summarizeCleanupErrors(cleanupStatus?.global.lastResult)}</p>
            <p className="muted compact">
              Reclaimed: {cleanupStatus?.global.lastResult?.pruneResult ? 'reported by Docker' : 'none'}
            </p>
          </div>
        </div>
      </section>
