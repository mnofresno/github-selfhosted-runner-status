...
export function TargetCard({ target, busy, onBusyChange, onStatusChange, onRefresh }: TargetCardProps) {
  ...
  async function reconcileTarget() {
    onBusyChange(true);
    onStatusChange(`Reconciling target ${target.id}...`);
    try {
      await api.reconcileTarget(target.id);
      await onRefresh();
      onStatusChange('Target reconciled and fleet status refreshed.');
    } catch (error) {
      onStatusChange(`Failed: ${(error as Error).message}`);
    } finally {
      onBusyChange(false);
    ...
    <section className="card target-card">
      <div className="section-head">
        ...
        <div className="toolbar">
          ...
          <button type="button" onClick={() => void reconcileTarget()} disabled={busy}>Reconcile target</button>
