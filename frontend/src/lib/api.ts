import type { CleanupStatus, FleetStatus, TargetFormPayload, TargetUpdatePayload, WorkflowJob } from '../types';
...
export const api = {
  ...
  getCleanupStatus(): Promise<CleanupStatus> {
    return readJson<CleanupStatus>('/api/admin/cleanup/status');
  },
  cleanupFleet(): Promise<any> {
    return readJson<any>('/api/admin/cleanup/fleet', { method: 'POST' });
  },
  cleanupGlobal(): Promise<any> {
    return readJson<any>('/api/admin/cleanup/global', { method: 'POST' });
  },
  ...
  reconcileTarget(targetId: string): Promise<void> {
    return readJson<void>(`/api/targets/${encodeURIComponent(targetId)}/reconcile`, { method: 'POST' });
  },
